...
> **Note**: The `branchId` parameter must be a UUID returned by the server in Step 0 or from
> `?include=activeBranches`. Passing a string name instead will return
> `400 Bad Request: Unrecognized key: "branchName"`. If the branch doesn't exist yet, create it via
> the Step 0 model API first.
